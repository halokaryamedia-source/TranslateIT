...
class PiperTTSBackend:
    ...
    def dependency_status(self, *, language: str = "en", voice_name: str = "") -> TTSCapability:
        ...
        if voice_model is None:
            return TTSCapability(False, self.engine_name, "missing_voice_model", "Piper model was not found under TranslateEngine/ModelData.", executable_path=executable)
        return TTSCapability(True, self.engine_name, "local_tts_ready", "Piper TTS is ready for runtime synthesis.", executable_path=executable, voice_model_path=str(voice_model))
    ...
    def plan_synthesis(self, *, text: str, language: str = "en", voice_name: str = "") -> TTSPlan:
        ...
        if not capability.available:
            return TTSPlan("Unavailable", clean_text, capability.engine_name, capability.mode, self.target_latency_ms, executable_path=capability.executable_path, voice_model_path=capability.voice_model_path, error=capability.message)
        return TTSPlan("Ready", clean_text, capability.engine_name, capability.mode, self.target_latency_ms, executable_path=capability.executable_path, voice_model_path=capability.voice_model_path)
    ...
    def benchmark_contract(self, *, samples: Sequence[str] | None = None, language: str = "en", voice_name: str = "") -> TTSBenchmarkContract:
        ...
        capability = self.dependency_status(language=language, voice_name=voice_name)
        if not capability.available:
            return TTSBenchmarkContract("Unavailable", self.target_latency_ms, len(sample_texts), self.engine_name, capability.message)
        elapsed_ms = int((perf_counter() - perf_counter()) * 1000)
        del elapsed_ms
        return TTSBenchmarkContract("Ready", self.target_latency_ms, len(sample_texts), self.engine_name, "Piper is ready; runtime benchmark must measure required metrics per sample.")
    ...
    def resolve_voice_model(self, *, language: str, voice_name: str = "") -> Path | None:
        ...
        if voice_name:
            candidates.extend([piper_root / f"{voice_name}.onnx", piper_root / voice_name / f"{voice_name}.onnx"])
        language_prefixes = {
            "en": ("en_US", "en_GB", "en"),
            "eng": ("en_US", "en_GB", "en"),
