...
class RealtimeReadinessAuditor:
    """Audit realtime TranslateIT readiness without starting capture or playback."""

    def __init__(self, model_root: Path | None = None) -> None:
        self.model_root = model_root
        self.asr_loader = ASRModelLoader(model_root=model_root)
        self.translation_engine = TranslationEngine(model_root=model_root)
    ...
    def audit(self, *, source_language: str = "id", target_language: str = "en") -> RealtimeReadinessAudit:
        ...
        items.append(
            RealtimeReadinessItem(
                "Streaming STT dependency",
                "PASS" if asr_dependency == "available" else "BLOCKED",
                True,
                "faster_whisper dependency is available." if asr_dependency == "available" else f"ASR dependency status: {asr_dependency}",
            )
        )
        ...
        items.append(
            RealtimeReadinessItem(
                "Fast MT backend",
                "PASS" if fast_mt_status.available else "BLOCKED",
                True,
                fast_mt_status.message,
            )
        )

        fallback_translation = self.translation_engine.dependency_status()
        items.append(
            RealtimeReadinessItem(
                "Fallback local translation",
                "PASS" if fallback_translation.available else "WARNING",
                False,
                fallback_translation.message,
            )
        )
        ...
        items.append(
            RealtimeReadinessItem(
                "Local TTS readiness",
                "PASS" if tts_status.available else "BLOCKED",
                True,
                tts_status.message,
            )
        )
        ...
        items.append(
            RealtimeReadinessItem(
                ...
                "PASS" if budget.target_total_ms <= 1250 else "WARNING",
                True,
                f"Target budget is {budget.target_total_ms} ms.",
            )
        )
