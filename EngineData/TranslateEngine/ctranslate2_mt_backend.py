...
class CTranslate2MTBackend:
    ...
    This backend is intentionally optional. If CTranslate2 or the converted
    local model is not present, callers must fall back to the existing local
    Transformer path without claiming that fast MT is active.
    """
    ...
    def dependency_status(self, engine_name: str) -> FastMTCapability:
        ...
        if model_path is None:
            return FastMTCapability(
                False,
                normalized,
                "missing_model",
                "Converted CTranslate2 model files are not available under TranslateEngine/ModelData.",
            )
    ...
    def load(self, engine_name: str) -> FastMTCapability:
        ...
        try:  # pragma: no cover - requires optional runtime dependencies and local model files
            import ctranslate2
