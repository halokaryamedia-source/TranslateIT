"""Pinned GPT-SoVITS V2ProPlus provider for My Voice build and trained-actor inference."""
...
from my_voice_training_runner import install_headless_my_utils
...
class MyVoiceProviderError(RuntimeError):
    pass


def require_file(path: Path, label: str) -> None:
    if not path.is_file() or path.stat().st_size <= 0:
        raise MyVoiceProviderError(f"missing_asset:{label}")
...
def validate_source_revision(source_root: Path) -> Path:
    ...
    if marker.read_text(encoding="utf-8").strip() != ENGINE_REVISION:
        raise MyVoiceProviderError("source_revision_mismatch")
    ...
    try:
        with wave.open(str(path), "rb") as reader:
            if reader.getnchannels() != 1 or reader.getsampwidth() != 2 or reader.getframerate() != 32_000:
                raise MyVoiceProviderError(f"noncanonical_take:{path.name}")
            frames = reader.getnframes()
    except MyVoiceProviderError:
        raise
    except Exception as exc:
        raise MyVoiceProviderError(f"invalid_take:{path.name}") from exc
    if frames <= 0:
        raise MyVoiceProviderError(f"empty_take:{path.name}")
    return frames * 1_000 // 32_000


def require_regular_file(path: Path, label: str) -> tuple[int, int]:
    if path.is_symlink() or not path.is_file():
        raise MyVoiceProviderError(f"invalid_actor_asset:{label}")
    stat = path.stat()
    if stat.st_size <= 0:
        raise MyVoiceProviderError(f"invalid_actor_asset:{label}")
...
def validate_actor_package(actor_dir: Path) -> dict[str, Any]:
    if actor_dir.is_symlink() or not actor_dir.is_dir():
        raise MyVoiceProviderError("approved_actor_missing")
    ...
    if manifest_size > MAX_ACTOR_MANIFEST_BYTES:
        raise MyVoiceProviderError("actor_manifest_size_invalid")
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise MyVoiceProviderError("actor_manifest_invalid_json") from exc
    if not isinstance(manifest, dict):
        raise MyVoiceProviderError("actor_manifest_invalid_json")
    if type(manifest.get("schema_version")) is not int or manifest["schema_version"] != ACTOR_SCHEMA_VERSION:
        raise MyVoiceProviderError("actor_schema_mismatch")
    if manifest.get("engine") != ENGINE or manifest.get("engine_revision") != ENGINE_REVISION:
        raise MyVoiceProviderError("actor_engine_contract_mismatch")
    if manifest.get("gpt_weight_file") != ACTOR_GPT_WEIGHT_FILE or manifest.get("sovits_weight_file") != ACTOR_SOVITS_WEIGHT_FILE or manifest.get("reference_wav_file") != ACTOR_REFERENCE_WAV_FILE:
        raise MyVoiceProviderError("actor_package_filename_mismatch")
    reference_text = str(manifest.get("reference_text", "")).strip()
    if not reference_text:
        raise MyVoiceProviderError("actor_reference_text_missing")
    if manifest.get("held_out_evaluation_complete") is not True:
        raise MyVoiceProviderError("actor_evaluation_incomplete")
    reference_duration = manifest.get("reference_duration_ms")
    if type(reference_duration) is not int:
        raise MyVoiceProviderError("actor_reference_duration_invalid")
...
def training_takes(dataset_dir: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in manifest["takes"]:
        if not isinstance(item, dict):
            raise MyVoiceProviderError("invalid_training_take")
    ...
    for item in manifest["held_out_lines"]:
        if not isinstance(item, dict):
            raise MyVoiceProviderError("invalid_held_out_line")
        line_id = int(item.get("line_id", 0))
        text = str(item.get("exact_text", "")).strip()
...
def select_reference(takes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not eligible:
        raise MyVoiceProviderError("reference_take_3_to_10_seconds_required")
...
def run_stage(source_root: Path, script: Path, env: dict[str, str], *args: str) -> None:
    ...
    runner = Path(__file__).resolve().with_name("my_voice_training_runner.py")
    ...
    if result.returncode != 0:
        raise MyVoiceProviderError(f"upstream_stage_failed:{script.name}:{result.returncode}")
...
def merge_part(source: Path, target: Path, header: str | None = None) -> None:
    ...
    if not body:
        raise MyVoiceProviderError(f"upstream_output_empty:{source.name}")
...
def checkpoint_epoch(path: Path, family: str) -> int:
    if family == "sovits":
        ...
        raise MyVoiceProviderError(f"unknown_candidate_family:{family}")
    if not match:
        raise MyVoiceProviderError(f"{family}_checkpoint_epoch_unparseable:{path.name}")
...
def epoch_weights(directory: Path, suffix: str, family: str) -> dict[int, Path]:
    ...
    for path in directory.glob(f"*{suffix}"):
        ...
        if epoch in result:
            raise MyVoiceProviderError(f"{family}_duplicate_checkpoint_epoch:{epoch}")
        result[epoch] = path
    if not result:
        raise MyVoiceProviderError(f"{family}_candidate_checkpoints_missing")
    return result
...
def nearest_epoch(checkpoints: dict[int, Path], total_epochs: int, progress_numerator: int) -> int:
    if not checkpoints or total_epochs <= 0:
        raise MyVoiceProviderError("candidate_checkpoint_set_invalid")
...
def select_training_candidates(sovits_dir: Path, gpt_dir: Path) -> list[dict[str, Any]]:
    ...
    for progress_numerator in range(1, MAX_TRAINING_CANDIDATES + 1):
        ...
        if pair in seen_pairs:
            continue
        seen_pairs.add(pair)
        candidates.append({"candidate_id": f"s{sovits_epoch}-g{gpt_epoch}", "candidate_order": len(candidates), "sovits_epoch": sovits_epoch, "gpt_epoch": gpt_epoch, "sovits_path": sovits[sovits_epoch], "gpt_path": gpt[gpt_epoch]})
    if len(candidates) < 2:
        raise MyVoiceProviderError(f"candidate_checkpoint_set_too_small:{len(candidates)}")
...
def write_wav(path: Path, sample_rate: int, audio: Any) -> None:
    ...
    if values.size == 0:
        raise MyVoiceProviderError("evaluation_audio_empty")
...
def create_tts_runtime(source_root: Path, assets: dict[str, Path], gpt_weight: Path, sovits_weight: Path, reference_wav: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise MyVoiceProviderError(f"cuda_probe_failed:{type(exc).__name__}") from exc
...
def evaluate_candidate(source_root: Path, assets: dict[str, Path], candidate: dict[str, Any], evaluation_root: Path, manifest: dict[str, Any], reference: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for held in manifest["held_out_lines"]:
            ...
            if len(outputs) != 1:
                raise MyVoiceProviderError(f"evaluation_output_count:{candidate_id}:{line_id}:{len(outputs)}")
            sr, audio = outputs[0]
            ...
            if not math.isfinite(score):
                raise MyVoiceProviderError(f"evaluation_similarity_invalid:{candidate_id}:{line_id}")
    ...
    if len(samples) != len(manifest["held_out_lines"]):
        raise MyVoiceProviderError(f"held_out_evaluation_incomplete:{candidate_id}")
...
def select_best_candidate(evidence: list[dict[str, Any]]) -> dict[str, Any]:
    if len(evidence) < 2 or len(evidence) > MAX_TRAINING_CANDIDATES:
        raise MyVoiceProviderError(f"candidate_evidence_count_invalid:{len(evidence)}")
    for candidate in evidence:
        ...
        if not isinstance(samples, list) or not samples:
            raise MyVoiceProviderError("candidate_evidence_samples_missing")
        if not isinstance(mean_similarity, (int, float)) or not math.isfinite(float(mean_similarity)):
            raise MyVoiceProviderError("candidate_evidence_mean_invalid")
        if not isinstance(minimum_similarity, (int, float)) or not math.isfinite(float(minimum_similarity)):
            raise MyVoiceProviderError("candidate_evidence_minimum_invalid")
...
def promote_selected_candidate(selected: dict[str, Any], candidate_dir: Path, evaluation_dir: Path, reference: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for sample in selected["samples"]:
        ...
        if sha256_file(target) != str(sample["sha256"]):
            raise MyVoiceProviderError(f"selected_evaluation_copy_hash_mismatch:{line_id}")
...
def synthesize_voice_actor(runtime: dict[str, Any], text: str, output_path: Path) -> dict[str, Any]:
    ...
    if tts is None or not isinstance(reference_wav, Path) or not reference_text:
        raise MyVoiceProviderError("voice_actor_runtime_invalid")
    outputs = list(tts.run(english_tts_inputs(text, reference_wav, reference_text)))
    if len(outputs) != 1:
        raise MyVoiceProviderError(f"inference_output_count:{len(outputs)}")
    sample_rate, audio = outputs[0]
    write_wav(output_path, int(sample_rate), audio)
    if not output_path.is_file() or output_path.stat().st_size <= 44:
        raise MyVoiceProviderError("inference_audio_invalid")
...
def build_candidate(*, source_root: Path, dataset_dir: Path, candidate_dir: Path, evaluation_dir: Path, work_dir: Path, manifest: dict[str, Any], status_writer: Callable[[str, str], None]) -> None:
    ...
    status_writer("training", "Creating bounded My Voice training candidates from accepted recordings.")
    candidates = train(source_root, assets, exp, candidate_dir)
    status_writer("evaluating", "Comparing held-out My Voice candidates before review.")
