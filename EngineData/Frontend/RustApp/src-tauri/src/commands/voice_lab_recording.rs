...
const GUIDED_LINES: &[(u32, &str)] = &[
    ...
    (2, "I'd like to review the project timeline before we make a final decision."),
    ...
    (5, "Our latest build is version two point six, and the remaining issue is still under review."),
    (6, "Please send the updated document after the meeting so I can check the details."),
    (7, "We can keep the existing approach if it stays stable and easy to maintain."),
    (8, "I agree with the main direction, but I want the final result to feel natural."),
    (9, "The development team will test the change before it goes into the release."),
    ...
    (12, "We should focus on reliability, clear communication, and consistent performance."),
    ...
    (15, "If something goes wrong, the system should recover safely instead of changing behavior without warning."),
    ...
    (21, "The bright morning light came through the window while we talked over breakfast."),
    (22, "She chose a fresh blue shirt, a warm gray jacket, and comfortable walking shoes."),
    (23, "Three reviewers checked the draft carefully and shared their feedback with the group."),
    ...
    (27, "The wireless connection was weak for a moment, but the audio stayed clear enough to continue."),
    ...
    (30, "Sometimes I speak softly, and sometimes my voice gets stronger when a point is important."),
    ...
    (43, "Please verify the username, project code, file path, and backup location before you continue."),
    (44, "After the update, the application opened quickly, connected normally, and responded without freezing."),
    (45, "Although the first attempt failed, the second one worked after we changed the input settings."),
    ...
    (54, "Why did the previous version behave differently even though the settings looked almost identical?"),
    ...
    (59, "We need a clear yes or no before Friday, otherwise the supplier can't reserve the equipment."),
    ...
    (86, "Take your time, and don't rush the answer."),
    ...
    (93, "Please restart the application and try again."),
    ...
    (102, "If we change the behavior now, we should also check that the documentation, tests, and interface still describe the same result."),
    (103, "The issue only appears after the application has been running for a long time, so a quick test may not reveal it."),
    (104, "I want the final experience to feel calm and predictable, even when something goes wrong and the system needs time to recover."),
    (105, "Could you compare the two recordings and tell me whether the second one sounds clearer, warmer, or simply louder than the first?"),
    (106, "Before we approve the release, let's test the full setup on a clean machine and make sure the main workflow works from start to finish."),
    ...
    (108, "The interface looks simple, but several steps happen in sequence before the final result is ready for the user."),
    ...
    (110, "The report covers the current status, completed work, remaining risks, and what we should do after the review."),
    (111, "I wasn't sure about that word at first, so I slowed down, said it again, and continued with the sentence."),
    (112, "A good conversation can include quick replies, normal explanations, and longer technical sentences without sounding forced."),
    ...
    (114, "The invoice total is two thousand nine hundred eighty-four dollars, including delivery, support, and replacement parts."),
    (115, "Version three point four point one fixes the connection bug, while version three point five adds a new diagnostic screen."),
    (116, "Please send the file to Daniel on the design team, copy Maria from operations, and mention that the deadline has moved to next Thursday."),
    ...
    (120, "I was surprised by the result at first, then relieved when we discovered that the problem came from a simple setting rather than damaged hardware."),
    (121, "If you disagree with my recommendation, that's completely fine. Tell me which assumption seems wrong, and we can review the evidence together."),
    (122, "The fastest solution isn't always the best one, especially when a shortcut creates extra work that someone else has to handle later."),
    (123, "When I explain a technical problem, I usually start with what the user noticed, then describe the cause, the fix, and any limitation that remains."),
    (124, "Please tell me if my voice stays clear when I speak slowly, at a normal pace, and a little faster in the same conversation."),
    (125, "We don't need to solve every possible future problem today. We only need to finish the work that actually matters right now."),
    ...
    (127, "Natural conversation can include hesitation, a quick correction, a short laugh, or a change in emphasis, while the important words still remain clear."),
    (128, "Thanks for taking the time to go through this with me. We've covered the main points, and I'm comfortable stopping here for today."),
];
