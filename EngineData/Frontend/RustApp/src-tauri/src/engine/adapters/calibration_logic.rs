...
pub fn run_calibration_logic(request: CalibrationLogicRequest) -> CalibrationLogicResult {
    ...
    let voiced_frame_ratio = if speech_rms > noise_floor_rms * 1.15 {
        1.0
    } else {
        0.0
    };
    ...
    let capture_allowed = !matches!(
        input_state.as_str(),
        "Too Quiet" | "No Signal" | "Too Loud / Clipping"
    );
...
fn classify_input_state(noise_floor_rms: f32, speech_rms: f32, peak_level: f32) -> String {
    ...
    if peak_level >= 0.99 {
        ...
        "No Signal".to_string()
    } else if speech_rms <= 0.0018_f32.max(noise_floor_rms * 0.55)
        && peak_level < 0.008
        && speech_gap < 0.0010
    {
        "Too Quiet".to_string()
...
fn peak(samples: &[f32]) -> f32 {
    samples
        .iter()
        .take(MAX_CALIBRATION_SAMPLES)
        .map(|value| safe_sample(*value).abs())
