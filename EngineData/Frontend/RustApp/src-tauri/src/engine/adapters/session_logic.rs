...
pub struct SessionMetricReport {
    ...
    pub historical_metric_not_trusted: bool,
}
...
pub fn build_session_metric_report(request: SessionMetricRequest) -> SessionMetricReport {
    ...
    let issue_signals = build_issue_signals(IssueSignalInputs {
        quality_status: &quality_status,
        measurement_status: if component_total > 0 {
            "MEASURED"
        } else {
            "UNAVAILABLE"
        },
        main_stage: &main_stage,
        main_reason: if component_total > 0 {
        ...
        missing_latency_ms,
        tts_queue_depth,
        voice_signal_state: &voice_signal_state,
        audio_verify: if audio_verify_ms > 0 {
            "endpoint_wait_ms"
        } else {
            "Not Run"
        },
    ...
    SessionMetricReport {
        ...
        historical_metric_not_trusted: true,
    }
}
...
fn build_issue_signals(inputs: IssueSignalInputs<'_>) -> IssueSignals {
    let mut triage_points = Vec::new();
    if !inputs.quality_status.is_empty() {
        triage_points.push(format!("quality={}", inputs.quality_status));
    }
    if !inputs.measurement_status.is_empty() {
        triage_points.push(format!("measurement={}", inputs.measurement_status));
    }
    if !inputs.main_stage.is_empty() && inputs.main_stage != "Not Run" {
        triage_points.push(format!("bottleneck={}", inputs.main_stage));
    }
    if inputs.missing_latency_ms > 0 {
        triage_points.push(format!("missing_latency={}ms", inputs.missing_latency_ms));
    }
    if let Some(value) = inputs.speech_end_to_voice_proxy_ms {
        triage_points.push(format!("voice_proxy={value}ms"));
    }
    if let Some(queue_depth) = inputs.tts_queue_depth {
        if queue_depth > 0 {
            triage_points.push(format!("tts_queue_depth={queue_depth}"));
        }
    }
    if inputs.voice_signal_state != "unavailable" {
        triage_points.push(format!("voice_signal={}", inputs.voice_signal_state));
    }
    if triage_points.is_empty() {
        triage_points.push("no_issue_signals".to_string());
    }
    let status = issue_status_label(inputs.quality_status, inputs.measurement_status);
    let triage_hint = triage_points.join("; ");
    IssueSignals {
        status,
        primary_stage: if inputs.main_stage.is_empty() {
            "Unknown".to_string()
        } else {
            inputs.main_stage.to_string()
        },
        primary_reason: if inputs.main_reason.is_empty() {
            "unavailable".to_string()
        } else {
            inputs.main_reason.to_string()
        },
        audio_verify: inputs.audio_verify.to_string(),
        stt: inputs.stt.to_string(),
        translate: inputs.translate.to_string(),
        tts: inputs.tts.to_string(),
        speech_end_to_voice_proxy_ms: inputs.speech_end_to_voice_proxy_ms,
        missing_latency_ms: inputs.missing_latency_ms,
        tts_queue_depth: inputs.tts_queue_depth,
        voice_signal_state: inputs.voice_signal_state.to_string(),
        triage_points,
        triage_hint,
    }
}
