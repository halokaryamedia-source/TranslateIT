...
pub fn build_latency_logic(request: LatencyLogicRequest) -> LatencyLogicReport {
    ...
    LatencyLogicReport {
        ...
        measurement_status: if first_voice.is_some() {
            "MEASURED"
        } else {
            "PARTIAL"
        }
        ...
        translation_ms: ns_to_ms(request.translation_start_ns, request.translation_end_ns)
            .unwrap_or(0),
        tts_ms: ns_to_ms(request.tts_start_ns, request.tts_audio_ready_ns).unwrap_or(0),
        playback_enqueue_to_start_ms: ns_to_ms(request.playback_request_ns, playback_start)
            .unwrap_or(0),
        official_voice_latency_end_event: end_event.to_string(),
        official_voice_latency_end_event_quality: if end_event.is_empty() {
            "missing"
        } else {
            "proxy"
        }
