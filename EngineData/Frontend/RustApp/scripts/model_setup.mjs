...
const prepScript = path.join(root, "EngineData", "Frontend", "RustApp", "scripts", "prepare_local_models.py");
...
function previousTarget(entry) {
  const previous = entry.previous_paths?.[0];
  if (previous) return path.join(root, previous);
  if (entry.model_id === "piper") return path.join(root, "EngineData", "Backend", "RuntimeAssets", "Voice", "Piper");
  return null;
}
...
function main() {
  ...
  for (const entry of manifest.models ?? []) {
    const previous = previousTarget(entry);
    const runtime = runtimeTarget(entry);
    if (previous && exists(previous) && !exists(runtime)) {
      mappings.push({ model_id: entry.model_id, source: sanitizePath(root, previous), target: sanitizePath(root, runtime), ...mklinkJunction(runtime, previous) });
    } else if (previous && exists(previous) && exists(runtime)) {
      mappings.push({ model_id: entry.model_id, source: sanitizePath(root, previous), target: sanitizePath(root, runtime), ok: true, skipped: true, note: "runtime_mapping_exists" });
