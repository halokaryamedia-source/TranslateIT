...
function chooseCandidate(entry, scan) {
  ...
  const previousPaths = (entry.previous_paths ?? []).map((item) => sanitizePath(root, path.join(root, item)));
  const matches = (scan.candidates ?? []).filter((candidate) => {
    const pathValue = candidate.candidate_path;
    return pathValue === expected || previousPaths.includes(pathValue) || pathValue.includes(entry.model_id);
  });
...
const items = (manifest.models ?? []).map((entry) => {
  ...
  const previousPath = candidate ? candidate.candidate_path !== sanitizePath(root, path.join(root, entry.expected_path)) : false;
  return {
