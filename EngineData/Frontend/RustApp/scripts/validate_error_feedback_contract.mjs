...
]) expect(controller, marker, "central notice feedback");

for (const marker of [
  "Type text before translating.",
  "Text is too long. Limit:",
  ...
  "Attached files did not contain readable text.",
...
]) expect(controller, marker, "finally reset state");

for (const marker of [
  "catch (error)",
  "errorMessage(error)",
  "frontend_bridge_error",
  "Translation command returned no message.",
  "Type text before translating.",
]) expect(facade, marker, "facade error fallback");

for (const marker of [
  "escapeHtml",
  "cleanDisplayText",
  "UNSAFE_DISPLAY_CHARS",
  "TRANSLATION_PENDING_MESSAGE",
  "is-pending",
]) expect(chatViews, marker, "result error/display safety");
...
console.log("Error feedback contract passed: user-facing notices, loading resets, error fallbacks, and result safety are covered.");
