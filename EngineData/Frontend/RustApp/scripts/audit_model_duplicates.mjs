...
const roots = [
  ...
  path.join(root, "UserData", "CacheData", "model_cache"),
];
...
function activeRuntimeTargets(manifest) {
  ...
  for (const model of manifest.models ?? []) {
    ...
    for (const previousPath of model.previous_paths ?? []) targets.add(path.join(root, previousPath));
  }
  return targets;
}
...
function main() {
  ...
  for (const [signature, dirs] of bySignature.entries()) {
    ...
    const safeDelete = stats.map((item) => ({
      ...
      reason: item.active_path ? "active_runtime_path" : item.path.includes("/UserData/CacheData/model_downloads/") ? "download_cache_candidate" : "previous_model_source_keep_if_in_use",
    }));
    groups.push({
      signature,
