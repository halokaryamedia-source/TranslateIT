...
| --- | --- | --- | --- |
| Quick validation | `npm run validate:quick` | Fast source-level app safety check. | Low |
...
- App package validators can stay here when they directly validate `RustApp` source, Tauri config, UI files, or app-specific contracts.
...
The script set is still intentionally backwards-compatible. The next cleanup step is to reduce user-facing documentation and contributor instructions to the profile commands above, while keeping old aliases until local validation confirms they are safe to retire.