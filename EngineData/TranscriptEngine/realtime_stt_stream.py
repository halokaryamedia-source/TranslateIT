...
class RealtimeSTTConfig:
    """Rolling-buffer partial STT settings for realtime conversation mode."""
...
class RealtimeSTTStream:
    """Realtime partial-transcript adapter around ASRModelLoader.

    This is not a new ASR model. It adds a low-latency rolling-buffer strategy
    so the app can emit partial transcripts before waiting for a full final
    segment. If faster-whisper/model files are missing, the result explicitly
    reports unavailable instead of pretending realtime STT is active.
    """
    ...
    def flush_final(self) -> RealtimeSTTPartial:
        ...
        if self._speech_ms < self.config.min_final_audio_ms or self._audio_ms <= 0:
            self.reset()
            return RealtimeSTTPartial(
                text="",
                status="Skipped",
                latency_ms=int((perf_counter() - started) * 1000),
                audio_window_ms=0,
                is_final=True,
                message="Not enough speech audio to finalize.",
            )
    ...
    def _transcribe_window(self, *, is_final: bool, started: float) -> RealtimeSTTPartial:
        ...
        return RealtimeSTTPartial(
            ...
            audio_window_ms=self._audio_ms,
            is_final=is_final,
            model_name=result.model_name,
            language=result.language,
            message=result.message,
            budget_passed=latency_ms <= self.config.max_partial_latency_ms if not is_final else True,
        )
