...
class RealtimeStatusPanelContract:
    ...
    This module does not modify widgets directly. It gives `app_main.py` a safe
    row contract to consume when the UI panel is patched.
    """
    ...
    def default_row(cls) -> RealtimeStatusPanelRow:
        return RealtimeStatusPanelRow(
            key=cls.row_key,
            label=cls.row_label,
            value="Realtime status pending",
            state="warning",
        )
    ...
    def from_payload(cls, payload: Mapping[str, Any] | None) -> RealtimeStatusPanelRow:
        ...
        return RealtimeStatusPanelRow(
            key=cls.row_key,
            label=cls.row_label,
            value=compact_text or "Realtime status unavailable",
            state=cls.state_for_status(status),
        )
