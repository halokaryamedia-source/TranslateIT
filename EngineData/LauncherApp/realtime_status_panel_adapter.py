...
class RealtimeStatusPanelAdapter:
    ...
    This module intentionally works with a generic mapping instead of importing
    Qt widgets, so it can be tested without launching the desktop app.
    """
    ...
    def build_row_from_payload(payload: dict[str, Any] | None) -> RealtimeStatusPanelRow:
        ...
        return RealtimeStatusPanelContract.from_payload(
            {
                "status": bridge_payload.status,
                "compact_text": bridge_payload.compact_text,
            }
        )
