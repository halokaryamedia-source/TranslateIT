...
function candidatePaths(entry) {
  const candidates = [entry.expected_path, ...(entry.legacy_paths ?? [])]
    .filter(Boolean)
...
function classify(entry) {
  ...
  const results = candidates.map((candidate) => {
    ...
    const legacy = candidate !== path.join(root, entry.expected_path);
  ...
  if (present) {
    status = present.legacy ? "PARTIAL" : "PASS";
    blocker = null;
    nextAction = present.legacy
  ...
  return {
    ...
    legacy_paths: entry.legacy_paths ?? [],
...
export function run() {
  ...
  const report = {
    ...
    items,
    blockers,
    legacy_model_roots: legacyModelRoots.map((value) => sanitizePath(root, value)),
