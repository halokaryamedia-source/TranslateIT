...
function helperLabel(helperStatus: HelperBridgeStatus | null): string { return helperStatus ? `${helperStatus.state} / ${helperStatus.runtime_claim}` : "status unavailable"; }
...
export function audioSettingsView(settings: RuntimeSettings): string {
  ...
  return settingsPage("Audio", "Manage microphone input, speaker output, volume, and voice behavior.", "settings-view--audio", `
    ${settingsCard("settings-card--audio", `${settingsGrid(`${factorySelectField("Microphone", "Choose microphone", "mic", "checkAudioInputButton")}${factorySelectField("Speaker", voiceEnabled ? "Choose speaker" : "Enable speaker", "speaker", "audioVoiceToggleButton")}${settingsField("Microphone Volume", '<div class="range-v22 mic-range"><span></span><i></i></div>')}${settingsField("Speaker Volume", '<div class="range-v22 speaker-range"><span></span><i></i></div>')}`)}<div class="mic-test-row-v22">${primaryButton("Mic Test", { id: "micTestButton" })}<div class="meter-v22"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div></div>`)}
    ${settingsSection("Voice", "Configure voice input behavior and input processing profile.")}
    ${settingsCard("settings-card--voice", settingsGrid(`${settingsField("Voice Profile", `${radioOption(null, "Normal", "Default microphone input without extra noise processing.", true)}${radioOption(null, "Noise", "Reduce background noise and prioritize speech clarity.")}`, "Choose the microphone processing profile.")}${settingsField("Voice Mode", `${radioOption("audioSensitivityButton", "Always On", "Voice input stays ready while the app is active.", realtimeActive, true)}${radioOption(null, "Push to Talk", "Voice input only activates while holding a selected key.")}`, "Choose how TranslateIT listens to voice input.")}`))}
    ${settingsSection("Advanced Audio Setting", "Reserved for future audio device options.")}${advancedEmpty()}
  `);
}
...
export function developerSettingsView(args: { progress: number; cpu: string; ram: string; gpu: string; gpuStatus: string; logRows: string; note: string; logsExpanded: boolean; engineGood: boolean; helperStatus: HelperBridgeStatus | null; audioStudioEvidence: AudioStudioValidationEvidence | null; }): string {
  ...
  return settingsPage("Developer", "Simple tools for monitoring runtime health and fixing common issues.", "settings-view--developer", `
    ...
    ${settingsSection("Validation Evidence", "Latest Audio Studio local validation summary, if available.")}
    ${settingsCard("settings-card--diagnostic", `<div class="developer-log-body" aria-label="Validation evidence status">${statusRow("Audio Studio evidence", evidenceLabel(args.audioStudioEvidence), evidenceTone(args.audioStudioEvidence))}${statusRow("Summary verifier", "npm run verify:audio-studio:summary <summary-json-path>", "neutral")}${statusRow("Architecture contracts", "npm run validate:architecture-contracts", "neutral")}</div><p class="diagnostic-note">${escapeHtml(evidenceDetail(args.audioStudioEvidence))}</p>`)}
