  ...
  app.innerHTML = `
    ...
    <main id="mainApp" class="app-shell is-hidden">
      ...
      <section id="homePage" class="workspace">
        ...
        <section class="composer-wrap"><input id="attachmentInput" class="attachment-input" type="file" accept=".txt,.md,.json,.csv,text/plain,text/markdown,application/json,text/csv" multiple aria-hidden="true" tabindex="-1" /><div class="composer"><button id="composerPlusButton" class="composer-icon" type="button" aria-label="Attach text files">${icon("plus")}</button><textarea id="messageInput" placeholder="Type Indonesian text to translate..." autocomplete="off" maxlength="2000" rows="1" aria-label="Text to translate" style="field-sizing: content;"></textarea><button id="microphoneButton" class="composer-icon emphasis" type="button" aria-label="Start voice recording">${icon("mic")}</button><button id="sendButton" class="send-button" type="button" aria-label="Send text">${icon("arrowUp")}</button></div><p class="composer-help">Type up to 2000 characters, attach up to 4 text files, or press the microphone button on the right to record speech locally.</p></section>
      </section>
