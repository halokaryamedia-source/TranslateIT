...
pub struct LiveAudioBufferStatusReport {
    ...
    pub requires_downmix_to_target: bool,
...
pub fn reset_live_audio_buffer(sample_rate_hz: u32, channels: u16) -> LiveAudioBufferStatusReport {
...
pub fn append_live_f32_samples(samples: &[f32], sample_rate_hz: u32, channels: u16) {
    append_mono_samples(&downmix_f32(samples, channels), sample_rate_hz, channels);
}

pub fn append_live_i16_samples(samples: &[i16], sample_rate_hz: u32, channels: u16) {
    let converted = samples
        .iter()
        .map(|sample| (*sample as f32 / i16::MAX as f32).clamp(-1.0, 1.0))
        .collect::<Vec<_>>();
    append_mono_samples(&downmix_f32(&converted, channels), sample_rate_hz, channels);
}

pub fn append_live_u16_samples(samples: &[u16], sample_rate_hz: u32, channels: u16) {
    let converted = samples
        .iter()
...
fn append_mono_samples(samples_mono: &[f32], sample_rate_hz: u32, channels: u16) {
    ...
    if window.sample_rate_hz != sample_rate_hz || window.channels != channels {
...
fn build_status(window: Option<&LiveAudioWindow>) -> LiveAudioBufferStatusReport {
    ...
    let requires_downmix_to_target = window.channels != TARGET_CHANNELS;
    let ready_for_vad = buffered_duration_ms >= MIN_VAD_MS && vad_result.accepted;
    let ready_for_segment_pipeline = ready_for_vad
        && buffered_duration_ms >= MIN_SEGMENT_MS
        && !requires_resample_to_target
        && !requires_downmix_to_target;
    ...
    } else if requires_resample_to_target {
        "live_audio_buffer:resample_required_before_segment_pipeline".to_string()
    } else if requires_downmix_to_target {
        "live_audio_buffer:downmix_required_before_segment_pipeline".to_string()
    ...
    let note = if ready_for_segment_pipeline {
        format!(
            "Live audio buffer is ready for segment pipeline. duration_ms={}, rms={:.5}, peak={:.5}",
            buffered_duration_ms, evidence.rms, evidence.peak
        )
    } else if ready_for_vad {
        format!(
            "Live audio has passed VAD, but segment pipeline remains blocked. duration_ms={}, blocker={}",
...
fn inactive_status(blocker: &str, note: &str) -> LiveAudioBufferStatusReport {
    ...
    LiveAudioBufferStatusReport {
        ...
        requires_downmix_to_target: false,
        ready_for_vad: false,
