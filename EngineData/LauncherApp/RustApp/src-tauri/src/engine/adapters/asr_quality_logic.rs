...
pub fn evaluate_asr_quality(request: AsrQualityLogicRequest) -> AsrQualityLogicDecision {
    ...
    if no_speech_probability >= 0.92
        && voiced_frame_ratio <= 0.02
        && !contains_focus_anchor(&tokens)
    {
        return reject("High no-speech probability", &normalized, "");
    }
    if request.audio_duration_ms < 120
        && request.sustained_speech_ms < 100
        && !contains_focus_anchor(&tokens)
    {
...
fn find_badword_match(tokens: &[String]) -> String {
    let simplified = tokens
        .iter()
        .map(|token| simplify_badword_token(token))
    ...
    for start in 0..simplified.len() {
        for phrase in &phrases {
            if start + phrase.len() <= simplified.len()
                && simplified[start..start + phrase.len()]
                    .iter()
                    .map(String::as_str)
...
fn looks_like_gibberish(
    ...
    if tokens.is_empty() || contains_focus_anchor(tokens) {
        return false;
    }
    ...
    let max_len = tokens
        ...
        .unwrap_or(0);
    if tokens.len() <= 2 {
        if avg_log_prob <= -0.45 && no_speech >= 0.08 && voiced_ratio <= 0.32 {
            return true;
        }
        if lang_prob > 0.0 && lang_prob <= 0.45 && avg_log_prob <= -0.15 {
            return true;
        }
        if max_len >= 7 && distinct_ratio <= 0.5 && avg_log_prob <= -0.12 {
            return true;
        }
    }
    tokens.len() <= 3
...
fn looks_looping(
    ...
    if tokens.len() >= 8 && repeated_trigram >= 2 && distinct <= 0.55 {
        return true;
    }
    if tokens.len() >= 8 && repeated_bigram >= 3 && distinct <= 0.85 {
        return true;
    }
    if tokens.len() >= 6 && (repeated_bigram >= 1 || repeated_trigram >= 1) && distinct <= 0.70 {
        return avg_log_prob <= -0.03
            || no_speech >= 0.05
            || voiced_ratio <= 0.20
            || (lang_prob > 0.0 && lang_prob <= 0.90);
    }
    false
}

fn repeated_windows(tokens: &[String], window_size: usize) -> usize {
    if window_size == 0 || tokens.len() < window_size * 2 {
        return 0;
    }
