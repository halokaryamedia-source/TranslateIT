...
use crate::engine::adapters::live_tts_boundary_logic::{analyze_live_tts_boundary, LiveTtsBoundaryReport};
...
pub fn analyze_live_runtime_pipeline_gate() -> LiveRuntimePipelineGateReport {
    ...
    let asr_ready = asr_decoder.ok && asr_decoder.transcript_text.as_ref().map(|text| !text.trim().is_empty()).unwrap_or(false);
    let translation_ready = translation.ok && translation.translated_text.as_ref().map(|text| !text.trim().is_empty()).unwrap_or(false);
    let tts_ready = tts_boundary.ok && tts_boundary.output_audio_ready;
    let playback_ready = tts_boundary.playback_ready;

    let completed_stage_count = [microphone_ready, asr_ready, translation_ready, tts_ready, playback_ready]
        .iter()
        .filter(|value| **value)
        .count() as u8;
    ...
    let blocker = if !microphone_ready {
        format!("pipeline_gate:microphone_segment_not_ready:{}", target_segment.blocker)
    } else if !asr_ready {
        format!("pipeline_gate:asr_not_ready:{}", asr_decoder.blocker)
    } else if !translation_ready {
        format!("pipeline_gate:translation_not_ready:{}", translation.blocker)
    } else if !tts_ready {
