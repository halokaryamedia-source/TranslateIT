...
pub fn plan_playback(request: PlaybackLogicRequest) -> PlaybackLogicResult {
    ...
    let Some(audio_path) = request.audio_path.clone().filter(|value| !value.trim().is_empty()) else {
        return result("Unavailable", "No replay audio path is available.", None, output_device_id, true, false, queue_depth, "none");
    };
    ...
    if path.extension().and_then(|value| value.to_str()).map(|value| value.to_lowercase()) != Some("wav".to_string()) {
        return result("Unsupported", "Only WAV playback is supported.", Some(audio_path_label), output_device_id, true, false, queue_depth, "unsupported");
    }
    if !request.backend_available {
        let used_default_output = output_device_id.is_none();
        return result("Unsupported", "Playback backend is not available.", Some(audio_path_label), output_device_id, used_default_output, false, queue_depth, "unavailable");
    }
...
fn playback_path_label(value: &str) -> String {
    ...
    let file_name = path.file_name().and_then(|value| value.to_str()).unwrap_or("audio.wav");
    format!("UserData/CacheData/audio_segments/{}", safe_label(file_name, "audio.wav"))
}
