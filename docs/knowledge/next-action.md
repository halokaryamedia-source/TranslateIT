...
- Audio pure-contract coverage proves the 2-second rolling mono buffer bound, source-format reset, max-two finalized incoming backlog with oldest eviction/FIFO retained order, shared Meeting sequence identity, and fail-closed finalized WAV staging validation. Meeting output uses a guarded facade over the native CPAL runtime; public-boundary tests now prove stale-generation rejection plus invalid RIFF, missing fmt/data chunks, truncation, unsupported channels/rates/layout/encoding, empty payload rejection, and valid PCM16 progression only as far as the prepared-device boundary without execution.
