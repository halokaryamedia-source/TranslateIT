...
9. Scenario A7 (2026-08-23): real-code fixture `test_milmmt_continuation_rejects_token_ceiling_without_eos` passed (`pytest` exit 0) against `_continuation`, asserting blocker `translation:output_hit_token_ceiling_without_eos`. Evidence: `UserData/LogData/RustAppValidation/a7_incomplete_generation_pytest.txt`. Dev-tree unit proof; the ended-without-EOS branch shares the same validator.
10. Full automated sweep (2026-08-23): `validate:quick` PASS; `cargo test` 47/47 after full target rebuild; smoke v9 A1–A7 green (only red = approved_actor_missing). Full `pytest tests/` shows 8 failures, all verified pre-existing at HEAD before the cleanup wave: 7× `test_voice_actor_inference.py` (`voice_actor_provider` attribute mismatch against the exec-composed worker module) and 1× gpu-probe monkeypatch gap on this CUDA host. Sweep evidence: `UserData/LogData/RustAppValidation/full_pytest_sweep.txt`.
