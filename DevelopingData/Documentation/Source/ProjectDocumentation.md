...
`TranslateIT.cmd` is a convenience root shortcut only. It must not start the dev server, browser route, Python UI, or worker directly. Developer mode remains inside `EngineData/Frontend/RustApp` through npm/Tauri commands.
...
The old separated asset roots are retired and are not part of the active engine route.
...
- Keep runtime app code under `EngineData/Frontend/RustApp`.
- Keep runtime model/Piper slots under `EngineData/Backend/RuntimeAssets`.
- Keep user data and generated evidence under `UserData`.
- Do not add older Python app engines under `EngineData`.
