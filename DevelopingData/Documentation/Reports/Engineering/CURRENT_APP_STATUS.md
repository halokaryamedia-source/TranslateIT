...
- `EngineData/Backend/README.md` now describes `RustApp` as the active app package folder.
- Runtime readiness guard now uses neutral wording in code.
- `validate:single-active-engine` is registered in the package validation scripts.
- Single active engine validator checks required active engine paths and blocks inactive engine paths such as `EngineData/TranscriptEngine`, `EngineData/TranslateEngine`, `EngineData/Backend/LocalWorker`, root `EngineData/Backend/RuntimeAssets`, Python launcher files under `EngineData/Frontend`, and removed helper/policy files.
