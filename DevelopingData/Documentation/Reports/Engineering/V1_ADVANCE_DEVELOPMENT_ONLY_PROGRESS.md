...
Progress development-only saat ini: sekitar **82%**.

Definisi:

- Yang dihitung: source features, contracts, bridge, evidence, blocker, diagnostics, orchestration, dan professional gate.
...
1. User-facing route/device selection surface.
2. Guarded real audio output route implementation.
3. Source Orchestration UI button connection.
4. Final UX cleanup for route flow outside Developer Diagnostics.

## New command added in this batch
...
1. Build the user-facing route/device selection surface.
2. Implement guarded real audio output route runtime.
