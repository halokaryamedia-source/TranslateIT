...
- Conversion version: `0.6.5-rust-input-preparation-boundary`
- Date: `2026-06-14`
- Status: Full-Rust target with native audio device discovery, input preparation boundary, calibration profile status, CUDA probe boundary, and ASR/translation adapter plans
...
The scaffold includes a clean frontend shell, Rust command bridge, runtime state contracts, config contracts, path discovery, settings persistence, logging, diagnostics, native audio device discovery through `cpal`, Rust input preparation boundary, calibration profile status, audio evidence/VAD contracts, native inference backend records, CUDA probe boundary, ASR/translation adapter plans, CUDA policy contracts, and explicit pending-state responses.
...
Tauri Frontend
  -> Rust Command API
     ...
     -> Rust Input Preparation Boundary
...
| Input preparation | Default device may exist but not satisfy target pipeline assumptions | Inspect default input against target 16 kHz mono pipeline before real processing |
...
- Rust input preparation boundary is now present.
...
This branch currently contains a Rust/Tauri scaffold, Rust runtime support modules, native audio device discovery, Rust input preparation boundary, audio calibration/evidence/VAD gate modules, native inference backend selection contracts, CUDA probe boundary, ASR/translation adapter plans, CUDA policy boundary, adapter contracts, and migration documentation. The runtime engine itself is not fully converted yet.
