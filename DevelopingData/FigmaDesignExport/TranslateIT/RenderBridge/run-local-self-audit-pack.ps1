...
try {
  ...
  if (!$healthOk) { Add-Content -Path $ExitCodes -Value 'health=1'; Write-Host 'Health check failed.' -ForegroundColor Red }
  else { Add-Content -Path $ExitCodes -Value 'health=0'; Write-Host 'Health OK' -ForegroundColor Green }
