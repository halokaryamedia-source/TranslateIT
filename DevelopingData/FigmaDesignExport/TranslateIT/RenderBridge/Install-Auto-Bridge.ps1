...
Write-Host '[INFO] Preparing strict V5 enhanced Render Bridge dependencies...' -ForegroundColor Cyan
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Settings $Settings -Principal $Principal -Description 'Starts the TranslateIT strict V5 enhanced local Render Bridge for Figma plugin website imports.' | Out-Null

Write-Host '[INFO] Starting strict V5 enhanced Render Bridge now...' -ForegroundColor Cyan
Start-ScheduledTask -TaskName $TaskName
Start-Sleep -Seconds 4

try {
  $health = Invoke-RestMethod 'http://127.0.0.1:8844/health' -TimeoutSec 3
  if (($health.publicVersion -eq 'Version 0.1 - Alpha') -and ($health.adapter -match 'v5') -and ($health.adapter -match 'enhanced')) {
    Write-Host '[DONE] Strict V5 enhanced Auto Bridge is installed and running.' -ForegroundColor Green
    Write-Host 'The Figma plugin can now import by website address automatically.' -ForegroundColor Green
    exit 0
  }
  Write-Host "[ERROR] Bridge responded but is not strict V5 enhanced. Adapter: $($health.adapter)" -ForegroundColor Red
  exit 1
} catch {
  Write-Host '[WARNING] Startup task installed, but strict V5 enhanced health check did not respond yet.' -ForegroundColor Yellow
