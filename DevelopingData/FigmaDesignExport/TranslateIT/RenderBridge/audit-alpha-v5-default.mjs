...
if (!renderer.includes('V5.3 measured DOM reconstruction')) failures.push('V5.3 measured DOM marker missing');
if (!renderer.includes('measuredElements')) failures.push('renderer is not using measuredElements payload');
if (!renderer.includes('Measured DOM Image')) failures.push('measured DOM image rendering missing');
if (!renderer.includes('Measured DOM Box')) failures.push('measured DOM box rendering missing');
if (!renderer.includes('Measured Text')) failures.push('measured DOM text rendering missing');
if (!renderer.includes('scaleRect')) failures.push('renderer is not using measured source rectangles');
if (renderer.includes('Low Opacity Alignment')) failures.push('renderer still uses low opacity screenshot guide in main output');
