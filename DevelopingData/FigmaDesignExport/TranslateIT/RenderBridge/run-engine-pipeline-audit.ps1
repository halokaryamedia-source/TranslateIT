...
try{Start-Sleep -Seconds 3; Step 'framework-contract' { node .\tests\test-framework-output-contract.mjs }; Step 'blueprint-framework-output' { node .\tests\test-blueprint-framework-output.mjs $TargetUrl }; Step 'figma-support-engines' { node .\tests\test-figma-support-engines.mjs $TargetUrl }; Step 'figma-engine-maturity' { node .\tests\test-figma-engine-maturity.mjs $TargetUrl }; Step 'engine-pipeline-readiness' { node .\tests\test-engine-pipeline-readiness.mjs $TargetUrl }; Step 'engine-preview-page' { node .\src\write-engine-preview-page.mjs .\reports }}finally{if($server -and !$server.HasExited){Stop-Process -Id $server.Id -Force -ErrorAction SilentlyContinue}}
...
Write-Host 'reports/translateit-figma-engine-maturity.json'
