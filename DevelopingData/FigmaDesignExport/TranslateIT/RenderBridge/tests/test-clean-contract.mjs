...
const files = {
  ...
  readiness: read(path.join(root, 'src', 'evaluate-self-audit-readiness.mjs')),
...
if (pkg.scripts?.['test:self-audit-readiness'] !== 'node ./src/evaluate-self-audit-readiness.mjs ./reports') failures.push('npm test:self-audit-readiness must run readiness evaluator');
...
for (const marker of ['renderFigmaSimPreview', 'source-screenshot-underlay', 'Editable Reconstruction', 'translateit-figma-sim-preview-latest.png', 'translateit-figma-sim-main-latest.png']) must('figmaSim', files.figmaSim, marker, `Figma simulation missing ${marker}`);
for (const marker of ['evaluateSelfAuditReadiness', 'Manual Figma Allowed', 'Reviewable', 'Not Ready', 'figma-sim-main-latest.png']) must('readiness', files.readiness, marker, `readiness evaluator missing ${marker}`);
for (const marker of ['evaluateSelfAuditReadiness', 'translateit-self-audit-readiness.json', 'Figma Simulation Main Preview']) must('reviewPage', files.reviewPage, marker, `review dashboard missing ${marker}`);
...
for (const marker of ['renderFigmaSimPreview', 'mainPngPath', 'compareSourceAndClonePreview', 'translateit-figma-sim-main-diff-latest.png']) must('figmaSimTest', files.figmaSimTest, marker, `Figma sim test missing ${marker}`);
