...
function buildCloneModel(model) {
  ...
  const bgLayers = sections.map((section) => ({
    id: `bg-${section.id}`,
    type: 'shape',
  ...
  return {
    ...
    layers: allLayers,
    assets: model.assets || [],
    uiLibrary: { grouping: 'section-first-source-geometry', editableText: true, editableImages: true, screenshotReferenceOnly: true },
    diagnostics: { sections: sections.length, layers: allLayers.length, textLayers: allLayers.filter((x) => x.type === 'text').length, imageLayers: allLayers.filter((x) => x.type === 'image').length, shapeLayers: allLayers.filter((x) => x.type === 'shape').length, buttonLayers: allLayers.filter((x) => x.type === 'button').length }
  };
}
...
async function buildPayload(targetUrl) {
  ...
  const cloneModel = buildCloneModel(designModel);
  const payload = ok({ source: { ...capture.source, screenshot: capture.source.screenshot }, designModel, cloneModel, diagnostics: { capture: { rawElements: capture.rawElements.length, assets: capture.assets.length }, layout: layout.stats, model: designModel.diagnostics, cloneModel: cloneModel.diagnostics } });
...
async function handleRender(req, res, url) {
  ...
  try { return sendJson(res, 200, await buildPayload(target)); } catch (err) { return sendJson(res, 500, error(err?.message || err)); }
}
async function handleAudit(req, res, url) {
  ...
  try {
    ...
    const report = { publicVersion: PUBLIC_VERSION, engine: ENGINE, engineBuild: ENGINE_BUILD, targetUrl: target, generatedAt: new Date().toISOString(), readyForFigmaTest: audit.visualReadiness === 'pass', audit, diagnostics: payload.diagnostics };
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/health') return sendJson(res, 200, ok({ adapter: 'clean-render-bridge', activeServer: 'server.mjs', activeRenderer: 'plugin/code.js', contract: 'cloneModel', cloneMode: 'layout-preserving-editable-clone', legacyActive: false }));
