...
import { matchDomToVisual } from './src/match-dom-visual.mjs';
...
function color(value, fallback = '') { const raw = String(value || '').trim(); if (!raw || raw === 'transparent' || raw === 'rgba(0, 0, 0, 0)') return fallback; if (/^#[0-9a-fA-F]{6}$/.test(raw)) return raw; const m = raw.match(/rgba?\(([^)]+)\)/); if (!m) return fallback; const p = m[1].split(',').map((x) => Number.parseFloat(x)); if (p.length < 3 || p[3] === 0) return fallback; return '#' + p.slice(0, 3).map((n) => Math.round(Math.max(0, Math.min(255, n))).toString(16).padStart(2, '0')).join(''); }
function area(rect) { return Math.max(0, rect?.w || 0) * Math.max(0, rect?.h || 0); }
function sectionName(section) { if (section.role === 'header') return 'Section / Header'; if (section.role === 'hero') return 'Section / Hero'; if (section.role === 'footer') return 'Section / Footer'; if (section.role === 'content') return 'Section / Content'; return section.name || `Section / ${section.role || 'Content'}`; }
function layerType(element) { if (element.type === 'image') return 'image'; if (element.type === 'button') return 'button'; if (element.type === 'container') return 'shape'; return 'text'; }
function layerName(element) { const role = clean(element.role || element.type || 'Layer'); const txt = clean(element.text || element.alt || element.name || ''); if (element.type === 'image') return `Image / ${clean(element.alt || element.name || role)}`; if (element.type === 'button') return `Button / ${txt.slice(0, 42)}`; if (element.type === 'container') return `Background / ${role}`; if (role === 'title') return 'Hero / Title'; if (role === 'section-title') return 'Section / Title'; if (role === 'nav-item') return `Navigation / ${txt.slice(0, 42)}`; if (role === 'footer-link') return `Footer Link / ${txt.slice(0, 42)}`; if (role === 'footer-text') return `Footer Text / ${txt.slice(0, 42)}`; return `${role} / ${txt.slice(0, 42)}`; }
function styleOf(element) { const s = element.style || {}; return { color: color(s.color, '#111827'), backgroundColor: color(s.backgroundColor, ''), fontSize: Number(s.fontSize || 14), fontWeight: Number(s.fontWeight || 400), fontFamily: s.fontFamily || 'Inter', lineHeight: Number(s.lineHeight || 0), borderRadius: Number(s.borderRadius || 0), textAlign: s.textAlign || 'left' }; }
function buildCloneModel(model, visualModel, visualMatching) {
