...
function normalizeUrl(value) { const raw = String(value || '').trim(); if (!raw) return ''; return /^https?:\/\//i.test(raw) ? raw : `https://${raw}`; }
async function getBrowser() { if (!browserPromise) browserPromise = chromium.launch({ headless: true }); return browserPromise; }
function union(rects) { const safe = rects.filter(Boolean).filter((r) => r.w > 0 && r.h > 0); if (!safe.length) return { x: 0, y: 0, w: 1, h: 1 }; const x = Math.min(...safe.map((r) => r.x)); const y = Math.min(...safe.map((r) => r.y)); const right = Math.max(...safe.map((r) => r.x + r.w)); const bottom = Math.max(...safe.map((r) => r.y + r.h)); return { x, y, w: right - x, h: bottom - y }; }
function expand(rect, pad) { return { x: rect.x - pad, y: rect.y - pad, w: rect.w + pad * 2, h: rect.h + pad * 2 }; }
function overlaps(a, b) { return a.x < b.x + b.w && a.x + a.w > b.x && a.y < b.y + b.h && a.y + a.h > b.y; }
function componentName(members, index) { const hasImage = members.some((l) => l.type === 'image'); const hasText = members.some((l) => l.type === 'text'); const hasButton = members.some((l) => l.role === 'button-bg' || l.role === 'button-label'); if (hasImage && hasText) return `Image Card ${String(index + 1).padStart(2, '0')}`; if (hasButton) return `Button Group ${String(index + 1).padStart(2, '0')}`; if (hasImage) return `Media ${String(index + 1).padStart(2, '0')}`; if (hasText) return `Content Group ${String(index + 1).padStart(2, '0')}`; return `Component ${String(index + 1).padStart(2, '0')}`; }
function buildComponents(section) {
  const layers = section.layers || [];
  const used = new Set();
  const components = [];
  const candidates = layers.filter((l) => l.type === 'image' || l.role === 'button-bg' || (l.type === 'box' && l.rect.w > 72 && l.rect.h > 32 && l.rect.w * l.rect.h > 2600)).sort((a, b) => (b.rect.w * b.rect.h) - (a.rect.w * a.rect.h));
  candidates.forEach((candidate) => {
    ...
    components.push({ id: `component-${components.length + 1}`, role: 'component', name: componentName(members, components.length), rect: expand(union(members.map((l) => l.rect)), 6), layers: members.sort((a, b) => a.order - b.order) });
  });
...
function clusterSections(layers, viewport, pageHeight) {
  ...
  for (const layer of important) {
    ...
    if (!current || gap > 120 || (layer.rect.h > 340 && layer.rect.y > current.rect.y + 100)) { current = { id: `section-${sections.length + 1}`, role: 'section', layers: [] }; sections.push(current); }
    current.layers.push(layer);
    current.rect = union(current.layers.map((item) => item.rect));
  }
  sections.forEach((section, index) => {
    section.layers = layers.filter((layer) => { const mid = layer.rect.y + layer.rect.h / 2; return mid >= section.rect.y - 32 && mid <= section.rect.y + section.rect.h + 32; });
