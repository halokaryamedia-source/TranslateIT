...
const expectedMode = 'translateit-design-clone-alpha';
const health = await getJson(`${bridge}/health`);
assert(health.mode === expectedMode, `Expected Alpha health mode, got ${health.mode}`);
assert(health.publicVersion === 'Version 0.1 - Alpha', `Expected public Alpha version, got ${health.publicVersion}`);
...
assert(payload.mode === expectedMode, `Expected Alpha payload mode, got ${payload.mode}`);
assert(payload.publicVersion === 'Version 0.1 - Alpha', `Expected public Alpha version, got ${payload.publicVersion}`);
...
assert(plan.sections.every((section) => Array.isArray(section.priorityText)), 'Missing priorityText on one or more rebuildPlan sections.');
assert(plan.sections.every((section) => section.contentBudget), 'Missing contentBudget on one or more rebuildPlan sections.');
...
assert(Array.isArray(plan.componentBlueprints) && plan.componentBlueprints.length >= 4, 'Missing or weak componentBlueprints.');
assert(plan.qualityHints && typeof plan.qualityHints.overflowRiskCount === 'number', 'Missing qualityHints.');
...
console.log(JSON.stringify({
  ...
  rebuildPlan: {
    ...
    priorityTextCoverage: plan.sections.filter((section) => Array.isArray(section.priorityText)).length + '/' + plan.sections.length,
    componentBlueprints: plan.componentBlueprints.length,
