...
function imageFitOf(element, asset) {
  const meta = element.imageMeta || {};
  return {
    objectFit: meta.objectFit || element.style?.objectFit || asset?.objectFit || 'cover',
    objectPosition: meta.objectPosition || element.style?.objectPosition || asset?.objectPosition || '50% 50%',
    naturalWidth: meta.naturalWidth || asset?.naturalWidth || asset?.width || element.rect?.w || 0,
    naturalHeight: meta.naturalHeight || asset?.naturalHeight || asset?.height || element.rect?.h || 0,
    renderedRatio: meta.renderedRatio || 0,
    naturalRatio: meta.naturalRatio || 0,
    aspectDrift: meta.aspectDrift || 0
  };
}

function textStyleOf(style) {
  return {
...
function normalizeElement(element, assets) {
  ...
  if (semantic === 'image') {
    ...
    return { ...base, type: 'image', role: 'image', name: `Image / ${clean(element.alt) || asset?.name || element.index}`, assetId: asset?.id || null, alt: clean(element.alt || asset?.name || ''), imageFit: imageFitOf(element, asset) };
  }
...
function removeOffCanvasTextFragments(elements, page) {
  const width = Math.max(1, page.width || 1440);
  return elements.filter((item) => {
    if (item.type !== 'text') return true;
    const rect = item.rect || {};
    const visibleWidth = Math.max(0, Math.min(width, rect.x + rect.w) - Math.max(0, rect.x));
    const visibleRatio = visibleWidth / Math.max(1, rect.w || 1);
    const role = item.role || '';
    const text = clean(item.text);
    if ((rect.x < -8 || rect.x + rect.w > width + 8) && visibleRatio < 0.55 && ['title', 'section-title', 'subheading', 'body', 'label'].includes(role)) return false;
    if (rect.x < -80 && text.length <= 80) return false;
    return true;
  });
}
...
function inferSectionIntent(section, elements) {
  ...
  const surfaceCount = inside.filter((item) => item.type === 'container').length;
  if (section.role === 'header') return 'navigation-bar';
  if (section.role === 'hero') return imageCount ? 'hero-with-media' : 'text-hero';
  if (section.role === 'footer') return surfaceCount ? 'footer-colored-surface-groups' : 'footer-link-groups';
  ...
  return surfaceCount ? 'surface-layout' : 'content-block';
}
...
function applySafetyLayout(model) {
  ...
  for (const item of elements.filter((item) => item.type === 'text')) {
...
export function buildDesignModel(layout) {
  ...
  const offCanvasFiltered = removeOffCanvasTextFragments(normalizedRaw, page);
  const unsafeFiltered = removeUnsafeTextLayers(offCanvasFiltered, page);
