...
function rawIndexOf(element) { const match = String(element.id || '').match(/^el-(\d+)$/); return match ? Number(match[1]) : 0; }
function explicitZIndex(element) { const raw = String(element.source?.zIndex || element.style?.zIndex || '').trim(); if (!raw || raw === 'auto') return null; const value = Number.parseInt(raw, 10); return Number.isFinite(value) ? value : null; }
function paintOrderOf(element, type) { const explicit = explicitZIndex(element); const rawIndex = rawIndexOf(element); if (explicit !== null) return explicit * 1000 + rawIndex; if (type === 'shape') return rawIndex - 200; return rawIndex; }
...
function imageFitOf(element) { const fit = element.imageFit || element.layout || {}; return { objectFit: fit.objectFit || 'cover', objectPosition: fit.objectPosition || '50% 50%', naturalWidth: fit.naturalWidth || 0, naturalHeight: fit.naturalHeight || 0, renderedRatio: fit.renderedRatio || 0, naturalRatio: fit.naturalRatio || 0, aspectDrift: fit.aspectDrift || 0 }; }
...
export function buildCloneModel(model, visualModel, visualMatching) {
  ...
  const sections = (model.sections || []).map((section) => { const surface = sectionSurfaceColor(section, sourceElements); return { id: section.id, role: section.role, name: sectionName(section), rect: section.rect, layerIds: [], surface }; }).sort((a, b) => ((a.rect && a.rect.y) || 0) - ((b.rect && b.rect.y) || 0));
  ...
  for (const element of sourceElements) {
    ...
    if (type === 'image') { layer.imageFit = imageFitOf(element); layer.layout = { objectFit: layer.imageFit.objectFit, objectPosition: layer.imageFit.objectPosition, clip: true }; imageFitLayerCount += 1; }
    layers.push(layer);
