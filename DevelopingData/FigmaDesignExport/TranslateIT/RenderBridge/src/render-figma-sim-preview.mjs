...
function baseLayerCss(layer, rect, zIndex, visualBacking) {
  const s = layer.style || {};
  const opacityBase = Number.isFinite(Number(s.opacity)) ? Math.max(0, Math.min(1, Number(s.opacity))) : 1;
  const opacity = visualBacking ? Math.min(opacityBase, 0.02) : opacityBase;
  const overflow = layer.type === 'text' || layer.type === 'button' ? safeCssKeyword(s.overflow || 'visible', 'visible') : 'hidden';
  return [
    'position:absolute', `left:${px(rect.x)}`, `top:${px(rect.y)}`, `width:${px(rect.w)}`, `height:${px(rect.h)}`,
    `z-index:${Math.max(2, zIndex)}`, `opacity:${opacity}`, `border-radius:${px((s.borderRadius || 0))}`, 'box-sizing:border-box', `overflow:${overflow}`
  ].join(';');
}

function textCss(layer, rect, zIndex, visualBacking, scale) {
  const s = layer.style || {};
  const lineHeight = s.lineHeight ? px((s.lineHeight || 0) * scale) : 'normal';
  const letterSpacing = Number.isFinite(Number(s.letterSpacing)) ? px((s.letterSpacing || 0) * scale) : 'normal';
  return [
    baseLayerCss(layer, rect, zIndex, visualBacking),
    `font-size:${px((s.fontSize || 14) * scale)}`,
...
function layerHtml(layer, sectionRect, scale, assets, zIndex, visualBacking) {
  const lr = scaleRect(layer.rect, scale);
  const sr = scaleRect(sectionRect, scale);
  const rect = { x: lr.x - sr.x, y: lr.y - sr.y, w: lr.w, h: lr.h };
  const s = layer.style || {};
  if (layer.type === 'shape') return `<div data-layer="${esc(layer.id)}" style="${baseLayerCss(layer, rect, zIndex, visualBacking)};background:${safeColor(s.backgroundColor, '#FFFFFF')}"></div>`;
  if (layer.type === 'image') {
    const asset = assetById(assets, layer.assetId);
    const src = imageDataUrl(asset);
    if (src) return `<img data-layer="${esc(layer.id)}" src="${src}" style="${baseLayerCss(layer, rect, zIndex, visualBacking)};${imageFitCss(layer)};display:block" />`;
    return `<div data-layer="${esc(layer.id)}" style="${baseLayerCss(layer, rect, zIndex, visualBacking)};background:#E5E7EB"></div>`;
  }
  if (layer.type === 'button') return `<div data-layer="${esc(layer.id)}" style="${textCss(layer, rect, zIndex, visualBacking, scale)};background:${safeColor(s.backgroundColor, '#111827')};display:flex;align-items:center;justify-content:center;color:${safeColor(s.color, '#FFFFFF')}">${esc(layer.text)}</div>`;
  return `<div data-layer="${esc(layer.id)}" style="${textCss(layer, rect, zIndex, visualBacking, scale)}">${esc(layer.text)}</div>`;
}
...
export function buildFigmaSimPreviewHtml(payload) {
  ...
  for (const section of sections) {
    ...
    const children = layers.filter((layer) => layer.sectionId === section.id).map((layer, index) => layerHtml(layer, section.rect, scale, clone.assets || [], index + 2, visualBacking)).join('');
    sectionHtml.push(`<section data-section="${esc(section.id)}" style="position:absolute;left:0;top:${px(sr.y)};width:${px(frameW)};height:${px(Math.max(1, sr.h))};overflow:hidden;z-index:3;opacity:${visualBacking ? 0.02 : 1}">${children}</section>`);
  }
  const looseLayers = layers.filter((layer) => !sectionMap.has(layer.sectionId)).map((layer, index) => layerHtml(layer, { x: 0, y: 0, w: sw, h: sh }, scale, clone.assets || [], index + 2, visualBacking)).join('');
  const screenshot = sourceScreenshotDataUrl(payload);
  const backing = visualBacking ? `<img data-layer="source-screenshot-underlay" src="${screenshot}" style="position:absolute;left:0;top:0;width:${px(frameW)};height:${px(frameH)};z-index:1;object-fit:fill;display:block;user-select:none;pointer-events:none" />` : '';
  return `<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;padding:0;background:#202020;font-family:Inter,Arial,sans-serif}.root{position:relative;width:${px(rootW)};height:${px(rootH)};background:#fff;overflow:hidden}.title{position:absolute;left:80px;top:30px;color:#111827;font-weight:700;font-size:28px}.note{position:absolute;left:80px;top:68px;color:#667085;font-size:12px}.main{position:absolute;left:${px(rootPad)};top:${px(rootTop)};width:${px(frameW)};height:${px(frameH)};background:${safeColor(page.background, '#FFFFFF')};overflow:hidden;isolation:isolate}</style></head><body><main class="root"><div class="title">${esc(page.title || 'Website')}</div><div class="note">Version 0.1 - Alpha / translateit-core / alpha-clean-1 - ${visualBacking ? 'visual-backed editable clone' : 'layout-preserving editable clone'}</div><div class="main">${backing}${sectionHtml.join('')}${looseLayers}</div></main></body></html>`;
}
