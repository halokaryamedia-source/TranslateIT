...
async function stabilizePage(page) {
  ...
  await page.addStyleTag({ content: `
    *,*::before,*::after{animation-duration:0.001s!important;animation-delay:0s!important;animation-iteration-count:1!important;transition-duration:0s!important;transition-delay:0s!important;scroll-behavior:auto!important;caret-color:transparent!important}
    video,canvas{animation:none!important}
  ` }).then(() => { diagnostics.animationsFrozen = true; }).catch(() => {});
  await page.evaluate(() => {
    try { document.documentElement.style.scrollBehavior = 'auto'; } catch {}
    try { document.querySelectorAll('video').forEach((video) => { video.pause(); video.currentTime = 0; }); } catch {}
    try { document.querySelectorAll('[loading="lazy"]').forEach((el) => el.setAttribute('loading', 'eager')); } catch {}
  }).catch(() => {});
  ...
  for (let i = 0; i <= steps; i += 1) {
    ...
    diagnostics.lazyScrollPasses += 1;
    await sleep(180);
  }
  await page.evaluate(() => Promise.all(Array.from(document.images || []).map(async (img) => {
    try { if (img.decode) await img.decode(); } catch {}
  }))).catch(() => {});
  const imageStats = await page.evaluate(() => {
    const images = Array.from(document.images || []);
    return { imageCount: images.length, imageLoadedCount: images.filter((img) => img.complete && img.naturalWidth > 0 && img.naturalHeight > 0).length, imageBrokenCount: images.filter((img) => img.complete && (!img.naturalWidth || !img.naturalHeight)).length };
  }).catch(() => ({ imageCount: 0, imageLoadedCount: 0, imageBrokenCount: 0 }));
  ...
  try {
    ...
    for (const item of imageElements) {
      try {
        ...
        const buffer = await handle.screenshot({ type: 'png', animations: 'disabled' });
        imageAssets.push({ id: `asset-image-${item.imageIndex}`, kind: 'image', imageIndex: item.imageIndex, name: item.alt || `Image ${item.imageIndex + 1}`, rect: item.rect, imageMeta: item.imageMeta || null, contentType: 'image/png', base64: toBase64(buffer), width: Math.round(box.width), height: Math.round(box.height), naturalWidth: item.imageMeta?.naturalWidth || Math.round(box.width), naturalHeight: item.imageMeta?.naturalHeight || Math.round(box.height), objectFit: item.imageMeta?.objectFit || 'cover', objectPosition: item.imageMeta?.objectPosition || '50% 50%' });
      } catch (_) {}
    }
    return { source: { url, finalUrl: raw.finalUrl, title: raw.title, viewport: raw.viewport, pageHeight: raw.pageHeight, screenshot, captureDiagnostics }, rawElements: raw.elements, assets: imageAssets };
