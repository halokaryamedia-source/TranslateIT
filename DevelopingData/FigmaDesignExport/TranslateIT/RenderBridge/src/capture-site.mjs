...
function isSliceCandidate(item, raw) {
  ...
  if (item.role === 'image' || item.role === 'input') return false;
...
export async function captureSite(inputUrl, options = {}) {
  ...
  try {
    ...
    const raw = await page.evaluate(() => {
      ...
      const inputTextOf = (el) => { const tag = String(el.tagName || '').toLowerCase(); if (!/^(input|select|textarea)$/i.test(tag)) return ''; if (tag === 'select') return clean(el.selectedOptions && el.selectedOptions[0] ? el.selectedOptions[0].textContent : el.value); return clean(el.value || el.getAttribute('value') || el.getAttribute('placeholder') || el.getAttribute('aria-label') || el.name || el.id || ''); };
      ...
      const elements = nodes.map((el, index) => {
        ...
        if (item.role === 'input') return item.area >= 300 && item.rect.w >= 24 && item.rect.h >= 12;
    ...
    captureDiagnostics.inputElementCount = raw.elements.filter((item) => item.role === 'input').length;
