...
function isSliceCandidate(item, raw) {
  ...
  const hasVisualSurface = item.role === 'container' || item.role === 'navigation' || item.role === 'footer' || (bg && !/transparent|rgba\(0, 0, 0, 0\)/i.test(bg)) || (bgImg && bgImg !== 'none');
...
export async function captureSite(inputUrl, options = {}) {
  ...
  try {
    ...
    return { source: { url, finalUrl: raw.finalUrl, title: raw.title, viewport: raw.viewport, pageHeight: raw.pageHeight, screenshot, captureDiagnostics }, rawElements: raw.elements, assets: imageAssets.concat(componentAssets) };
