...
async function main() {
  ...
  payload.diagnostics.nativeUsefulness = { ...(payload.diagnostics.nativeUsefulness || {}), responsiveVariants: payload.responsiveRenderPlan.diagnostics?.variants || 0, responsiveLayoutIntentStatus: payload.responsiveLayoutIntent.status, styleInventoryStatus: payload.styleInventory.status, componentSummaryStatus: payload.componentSummary.status, layerNamePass: payload.figmaRenderPlan.diagnostics?.layerNamePass === true, pluginRenderFinalPass: payload.figmaRenderPlan.diagnostics?.pluginRenderFinalPass === true };
  await fs.mkdir(path.dirname(output), { recursive: true });
  await fs.writeFile(output, JSON.stringify(payload, null, 2));
  console.log(JSON.stringify({ status: 'written', output, renderPlanStatus: payload.figmaRenderPlan?.status, responsiveVariants: payload.responsiveRenderPlan?.diagnostics?.variants || 0, responsiveLayoutIntentStatus: payload.responsiveLayoutIntent?.status || 'missing', pluginRenderFinalPass: payload.figmaRenderPlan?.diagnostics?.pluginRenderFinalPass === true }, null, 2));
}
