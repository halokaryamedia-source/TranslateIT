...
async function main() {
  ...
  console.log(JSON.stringify({ status: 'written', targetUrl, output, renderPlanStatus: payload.figmaRenderPlan?.status, productionManifestStatus: payload.productionExportManifest?.status, productionRiskLevel: payload.productionExportManifest?.risk?.level || 'unknown', renderPlanSanitized: payload.figmaRenderPlan?.diagnostics?.sanitized === true }, null, 2));
}
