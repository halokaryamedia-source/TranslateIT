...
async function main() {
  ...
  payload.diagnostics.nativeUsefulness = { ...(payload.diagnostics.nativeUsefulness || {}), responsiveVariants: payload.responsiveRenderPlan.diagnostics?.variants || 0, styleInventoryStatus: payload.styleInventory.status, componentSummaryStatus: payload.componentSummary.status };
  await fs.mkdir(path.dirname(output), { recursive: true });
  await fs.writeFile(output, JSON.stringify(payload, null, 2));
  console.log(JSON.stringify({ status: 'written', targetUrl, output, renderPlanStatus: payload.figmaRenderPlan?.status, productionManifestStatus: payload.productionExportManifest?.status, productionRiskLevel: payload.productionExportManifest?.risk?.level || 'unknown', renderPlanSanitized: payload.figmaRenderPlan?.diagnostics?.sanitized === true, responsiveVariants: payload.responsiveRenderPlan?.diagnostics?.variants || 0, styleInventoryStatus: payload.styleInventory?.status || 'missing', componentSummaryStatus: payload.componentSummary?.status || 'missing' }, null, 2));
}
