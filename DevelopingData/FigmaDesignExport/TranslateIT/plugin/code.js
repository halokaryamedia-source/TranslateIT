...
const INTERNAL_RENDERER = 'translateit-alpha-final-clone-first-renderer';
...
const px = (v, fallback) => { const m = String(v || '').match(/-?\d+(\.\d+)?/); return m ? Number(m[0]) : fallback; };
...
function defaultResponsive() { return { desktop: 'Desktop uses full width sections.', tablet: 'Tablet reduces grids.', mobile: 'Mobile stacks content.' }; }
...
function imagePreview(parent, layer, x, y, w, h, radius) {
  const node = figma.createRectangle();
  node.name = safe('Image Preview / ' + clean(layer && layer.name || 'Media'));
  node.x = x || 0; node.y = y || 0; node.resize(w, h); node.cornerRadius = radius == null ? 18 : radius; node.strokes = [];
...
function chip(parent, text, x, y, fill, color) { rect(parent, 'Chip Background', x, y, Math.max(80, clean(text).length * 7 + 24), 28, fill, 14); placeText(parent, 'Chip Label', text, x + 12, y + 8, 9, color || '#FFFFFF', true, Math.max(60, clean(text).length * 7)); }
function shadowCard(parent, name, x, y, w, h, fill, radius, border) { const bg = rect(parent, name + ' Shadow', x + 8, y + 10, w, h, '#CBD5E1', radius || 24); bg.opacity = 0.18; return rect(parent, name + ' Surface', x, y, w, h, fill || '#FFFFFF', radius || 24, border || '#E5E7EB'); }
function normalizeIntent(v, fallback) { const raw = key(v || fallback || ''); if (raw.includes('header') || raw.includes('navigation')) return 'header'; if (raw.includes('hero') || raw.includes('landing')) return 'hero'; if (raw.includes('gallery') || raw.includes('grid') || raw.includes('card')) return 'gallery'; if (raw.includes('footer')) return 'footer'; return 'content'; }
...
function planTexts(planSection, section, limit) { const fromPlan = arr(planSection && planSection.priorityText).map((x) => ({ text: clean(x.text), role: x.role || 'text' })).filter((x) => x.text); return (fromPlan.length ? fromPlan : fallbackTexts(section, limit)).slice(0, limit || 8); }
function textList(items) { return arr(items).map((x) => typeof x === 'string' ? x : x.text).filter(Boolean); }
function sectionImages(section, fallbackImages, index, limit) { const local = arr(section.layers).filter((l) => l.type === 'image'); return (local.length ? local : arr(fallbackImages).slice(index, index + 1)).slice(0, limit || 2); }
...
function makeScreenshotFrame(payload, width) {
  ...
  if (payload.screenshot && payload.screenshot.base64) {
    ...
    shot.name = 'Pure Screenshot Reference / locked'; shot.resize(width, h); shot.fills = [{ type: 'IMAGE', imageHash: img.hash, scaleMode: 'FILL' }]; shot.strokes = [];
    try { shot.locked = true; } catch (_) {}
    f.appendChild(shot);
  }
  return f;
}
function headerTemplate(section, texts, images, planSection, t) { const block = frame((section.name || 'Header') + ' / Final Header', 1180, 118, '#FFFFFF'); block.cornerRadius = 28; block.strokes = paint(t.border); block.strokeWeight = 1; rect(block, 'Header Accent', 0, 0, 1180, 8, t.primary, 0); const list = textList(texts); placeText(block, 'Brand', cut(list[0] || 'Brand', 30), 34, 44, 23, t.text, true, 270); (list.slice(1, 6).length ? list.slice(1, 6) : ['Home', 'About', 'Projects', 'Contact']).forEach((item, i) => placeText(block, 'Nav Item', cut(item, 18), 380 + i * 116, 50, 12, '#475569', false, 100)); rect(block, 'Header CTA', 1000, 38, 118, 40, t.dark, 20); placeText(block, 'CTA Label', 'Contact', 1036, 50, 11, '#FFFFFF', true, 70); return block; }
function heroTemplate(section, texts, images, planSection, t) { const block = frame((section.name || 'Hero') + ' / Final Hero', 1180, 600, '#FFFFFF'); block.cornerRadius = 32; block.strokes = paint(t.border); block.strokeWeight = 1; rect(block, 'Hero Accent Panel', 680, 52, 430, 470, t.soft, 32); const list = textList(texts); chip(block, section.name || 'Hero Section', 44, 72, t.soft, t.primary); placeText(block, 'Hero Heading', cut(list[0] || 'Generated Hero Heading', (planSection.contentBudget || {}).headingChars || 90), 44, 126, 56, t.text, true, 560); placeText(block, 'Hero Body', cut(list.slice(1, 4).join(' ') || 'Editable supporting copy generated from the source website structure.', (planSection.contentBudget || {}).bodyChars || 180), 48, 310, 17, t.muted, false, 520); rect(block, 'Primary CTA', 48, 430, 152, 48, t.primary, 24); placeText(block, 'CTA Text', 'Explore', 96, 445, 12, '#FFFFFF', true, 84); rect(block, 'Secondary CTA', 220, 430, 150, 48, '#FFFFFF', 24, t.border); placeText(block, 'Secondary CTA Text', 'Learn More', 258, 445, 12, t.text, true, 96); if (images.length) imagePreview(block, images[0], 710, 86, 360, 370, 30); else rect(block, 'Hero Media Placeholder', 710, 86, 360, 370, '#E0E7FF', 30, '#CBD5E1'); return block; }
function contentTemplate(section, texts, images, planSection, t) { const block = frame((section.name || 'Content') + ' / Final Content', 1180, 430, '#FFFFFF'); block.cornerRadius = 30; block.strokes = paint(t.border); block.strokeWeight = 1; rect(block, 'Side Accent', 0, 0, 12, 430, t.primary, 0); const list = textList(texts); chip(block, section.name || 'Content Section', 42, 44, t.soft, t.primary); placeText(block, 'Content Heading', cut(list[0] || 'Editable Content Section', (planSection.contentBudget || {}).headingChars || 80), 42, 94, 36, t.text, true, 600); placeText(block, 'Content Body', cut(list.slice(1, 6).join(' ') || 'Editable body text generated from the page content.', (planSection.contentBudget || {}).bodyChars || 260), 44, 190, 15, t.muted, false, 600); if (images.length) imagePreview(block, images[0], 770, 70, 320, 260, 26); else shadowCard(block, 'Content Media Placeholder', 770, 70, 320, 260, '#F1F5F9', 26, '#D9E2F2'); return block; }
function galleryTemplate(section, texts, images, planSection, t) { const block = frame((section.name || 'Gallery') + ' / Final Gallery', 1180, 520, '#FFFFFF'); block.cornerRadius = 30; block.strokes = paint(t.border); block.strokeWeight = 1; const list = textList(texts); chip(block, section.name || 'Gallery', 40, 42, t.soft, t.primary); placeText(block, 'Gallery Heading', cut(list[0] || 'Editable Gallery Section', 80), 40, 86, 36, t.text, true, 720); const cardTexts = list.slice(1, 7).length ? list.slice(1, 7) : ['Card One', 'Card Two', 'Card Three']; for (let i = 0; i < 3; i += 1) { const x = 40 + i * 372; shadowCard(block, 'Gallery Card BG', x, 160, 336, 286, '#FFFFFF', 26, t.border); if (images[i]) imagePreview(block, images[i], x + 18, 180, 300, 128, 20); else rect(block, 'Card Media Placeholder', x + 18, 180, 300, 128, '#E5E7EB', 20); placeText(block, 'Card Title', cut(cardTexts[i] || ('Card ' + (i + 1)), 36), x + 22, 326, 15, t.text, true, 286); placeText(block, 'Card Body', 'Editable card description.', x + 22, 356, 11, t.muted, false, 286); } return block; }
function footerTemplate(section, texts, images, planSection, t) { const block = frame((section.name || 'Footer') + ' / Final Footer', 1180, 188, t.dark); block.cornerRadius = 30; block.strokes = paint('#1F2937'); block.strokeWeight = 1; const list = textList(texts); placeText(block, 'Footer Brand', cut(list[0] || 'Footer', 42), 40, 50, 24, '#F8FAFC', true, 420); placeText(block, 'Footer Note', cut(list.slice(1, 5).join(' ') || 'Editable footer content and closing navigation.', 160), 40, 96, 12, '#CBD5E1', false, 560); (list.slice(1, 5).length ? list.slice(1, 5) : ['Home', 'About', 'Contact']).forEach((item, i) => placeText(block, 'Footer Link', cut(item, 18), 760 + i * 100, 84, 11, '#CBD5E1', false, 86)); return block; }
function makeFinalCloneFrame(payload, plan, sections, layers) {
  const t = theme(plan);
  const f = autoFrame('01 Final Design Clone / Main Output', 1280, '#FFFFFF', 30, 40);
  f.cornerRadius = 32; f.strokes = paint(t.border); f.strokeWeight = 1;
  f.appendChild(label('Final Output Title', 'Final Design Clone / Main Output', 36, t.text, true, 1180));
  f.appendChild(label('Final Output Note', 'This is the primary editable clone result. Screenshot, library, plan, and audit are supporting frames below.', 13, t.muted, false, 1180));
  const globalImages = layers.filter((l) => l.type === 'image');
  plan.sections.slice(0, 10).forEach((planSection, index) => {
    const section = sections[index] || { name: planSection.name, layers: [] };
    const intent = normalizeIntent(planSection.templateIntent || planSection.intent, 'content');
    const texts = planTexts(planSection, section, 10);
    const images = sectionImages(section, globalImages, index, 4);
    if (intent === 'header') f.appendChild(headerTemplate(section, texts, images, planSection, t));
    else if (intent === 'hero') f.appendChild(heroTemplate(section, texts, images, planSection, t));
    else if (intent === 'gallery') f.appendChild(galleryTemplate(section, texts, images, planSection, t));
    else if (intent === 'footer') f.appendChild(footerTemplate(section, texts, images, planSection, t));
    else f.appendChild(contentTemplate(section, texts, images, planSection, t));
  });
  return f;
}
function makePlanFrame(plan) {
  const t = theme(plan); const f = autoFrame('04 Rebuild Plan / AI Interpretation', 1280, '#FFFFFF', 20, 32); f.cornerRadius = 28; f.strokes = paint(t.border); f.strokeWeight = 1;
  f.appendChild(label('Plan Title', 'Rebuild Plan / AI Interpretation', 30, t.text, true, 1160));
  f.appendChild(label('Plan Summary', plan.summary, 13, t.muted, false, 1160));
  f.appendChild(label('Plan Counts', 'Sections: ' + plan.counts.sections + ' | Text: ' + plan.counts.text + ' | Images: ' + plan.counts.images + ' | Buttons: ' + plan.counts.buttons, 13, t.primary, true, 1160));
  const grid = frame('Section Blueprint Grid', 1180, Math.ceil(Math.max(1, plan.sections.length) / 3) * 156, null); grid.fills = [];
  plan.sections.forEach((section, index) => { const card = frame('Blueprint Card / ' + (section.name || index), 372, 140, index % 2 ? '#FFFFFF' : '#F8FAFC'); card.cornerRadius = 20; card.strokes = paint(section.overflowRisk ? '#FCA5A5' : t.border); card.strokeWeight = 1; card.x = (index % 3) * 394; card.y = Math.floor(index / 3) * 156; chip(card, section.templateIntent || 'content', 18, 14, section.overflowRisk ? '#DC2626' : t.primary, '#FFFFFF'); placeText(card, 'Section Name', section.name || ('Section ' + (index + 1)), 18, 52, 15, t.text, true, 330); placeText(card, 'Layout', (section.suggestedLayout || 'default') + ' / ' + (section.density || 'normal'), 18, 78, 10, t.muted, true, 330); placeText(card, 'Priority Text', cut(textList(section.priorityText).slice(0, 2).join(' / ') || 'No priority text', 94), 18, 104, 9, t.muted, false, 330); grid.appendChild(card); });
...
function makeLibraryFrame(plan, layers) { const t = theme(plan); const lib = autoFrame('03 UI Library / Supporting Design System', 1280, t.bg, 28, 32); lib.appendChild(label('Library Title', 'UI Library / Supporting Design System', 32, t.text, true, 1160)); lib.appendChild(label('Library Note', 'Supporting tokens and components. Main clone output is the first frame above.', 12, t.muted, false, 1160)); appendMiniGrid(lib, 'Color Tokens', plan.tokens.colors.length ? plan.tokens.colors : ['#111827', '#F8FAFC', t.primary], tokenComponent, 7, 156, 100); appendMiniGrid(lib, 'Component Blueprints', plan.componentBlueprints.length ? plan.componentBlueprints : [{ name: 'Card / Default' }], (item, i) => { const c = frame('Blueprint / ' + (item.name || i), 260, 112, '#FFFFFF'); c.cornerRadius = 20; c.strokes = paint('#D9DEE8'); c.strokeWeight = 1; placeText(c, 'Blueprint Name', item.name || 'Component', 16, 18, 13, '#111827', true, 220); placeText(c, 'Blueprint Purpose', cut(item.purpose || 'Reusable component.', 80), 16, 46, 10, '#64748B', false, 220); return c; }, 4, 260, 112); return lib; }
function makeAuditFrame(diag, plan) { const f = autoFrame('05 Audit / Design Clone Notes', 1280, '#0B1020', 14, 32); f.appendChild(label('Audit Title', 'Design Clone Audit / Honest Readiness', 28, '#F8FAFC', true, 1160)); f.appendChild(label('Audit Note', 'Final clone is now the primary output. Figma visual validation is still required before 9+ claims.', 13, '#CBD5E1', false, 1160)); f.appendChild(label('Audit Diagnostics', 'sections: ' + diag.sectionCount + ' / layers: ' + diag.layerCount + ' / images: ' + diag.imageCount + ' / text: ' + diag.textCount + ' / colors: ' + diag.colorTokenCount + ' / type: ' + diag.textStyleCount + ' / spacing: ' + diag.spacingTokenCount + ' / radius: ' + diag.radiusTokenCount + ' / blueprints: ' + diag.componentBlueprintCount + ' / overflow risk: ' + diag.overflowRiskCount + ' / renderer: ' + INTERNAL_RENDERER, 14, '#93C5FD', true, 1160)); return f; }
...
async function importUniversal(payload) {
  ...
  run.appendChild(label('Run Note', PUBLIC_VERSION + ': final clone first, screenshot and library are supporting references.', 12, '#8D96A6', false, 1320));
  const finalClone = makeFinalCloneFrame(payload, plan, sections, layers);
  run.appendChild(finalClone);
