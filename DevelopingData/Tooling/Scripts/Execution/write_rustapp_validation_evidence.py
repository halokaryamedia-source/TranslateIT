...
MANUAL_DEFAULTS = {"microphone_capture_smoke_test": False, "asr_transcript_smoke_test": False, "translation_smoke_test": False, "tts_playback_smoke_test": False, "launcher_package_open_test": False}
...
def worker_smoke_summary() -> dict:
    ...
    if not smoke:
        return {"loaded": False, "ok": False, "evidence_path": str(WORKER_SMOKE_EVIDENCE.relative_to(ROOT)).replace("\\", "/")}
    return {"loaded": True, "ok": bool(smoke.get("ok", False)), "schema": smoke.get("schema"), "generated_at_utc": smoke.get("generated_at_utc"), "mode": smoke.get("mode"), "persistent_worker": bool(smoke.get("persistent_worker", False)), "latency_summary": smoke.get("latency_summary", {}), "evidence_path": str(WORKER_SMOKE_EVIDENCE.relative_to(ROOT)).replace("\\", "/")}
...
def main() -> int:
    ...
    evidence = {
        ...
        "rust_check_passed": bool_arg(sys.argv[1]),
        "frontend_typecheck_passed": bool_arg(sys.argv[2]),
        "frontend_build_passed": bool_arg(sys.argv[3]),
        "tauri_build_passed": bool_arg(sys.argv[4]),
        "packaging_validation_passed": bool_arg(sys.argv[5]),
        "local_worker_stack_passed": bool_arg(sys.argv[6]) if len(sys.argv) >= 7 else False,
        "local_worker_smoke_evidence": worker_smoke_summary(),
        "manual_runtime_evidence": preserved_manual_evidence(),
