...
TOOLING_EXECUTION = ROOT / "DevelopingData" / "Tooling" / "Scripts" / "Execution"
SMOKE_SCRIPT = TOOLING_EXECUTION / "run_local_realtime_worker_smoke_tests.py"
MODEL_CHECK_SCRIPT = TOOLING_EXECUTION / "check_local_runtime_models.py"
...
REQUIRED_REQUIREMENTS = ["faster-whisper", "ctranslate2", "transformers", "torch", "sentencepiece"]
REQUIRED_SETUP_TERMS = ["requirements-realtime.txt", "realtime_local_worker.py", "check_local_runtime_models.py", "python -m venv", "pip install"]
REQUIRED_SMOKE_TERMS = ["PersistentWorker", "stream_reader", "threading.Thread", "queue.Queue", "persistent_worker", "threaded_non_blocking_stdout_queue", "latency_summary", "latest_local_worker_smoke_evidence.json", "asr_transcript_smoke", "translation_smoke", "tts_synthesis_smoke"]
REQUIRED_WORKER_COMMANDS = ["status", "asr_preload", "transcribe", "translation_preload", "translate", "tts_preflight", "synthesize"]
REQUIRED_MODEL_CHECK_TERMS = ["asr_faster_whisper_large_v3_turbo", "translation_marianmt_id_en", "translation_nllb_200_distilled_600m", "voice_piper"]
...
def main() -> int:
    ...
    for command in REQUIRED_WORKER_COMMANDS:
        if command not in manifest.get("worker_commands", {}):
    ...
    print("PASS: Local realtime worker stack files, setup, model checker, persistent smoke scripts, guarded paths, GPU-aware fallback, commands, dependencies, modes, and truth policy are present")
    return 0
