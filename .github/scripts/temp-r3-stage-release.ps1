...
$Temp = Join-Path $env:RUNNER_TEMP 'translateit-r3-stage'
...
function Assert-Hash([string]$Path, [string]$Expected, [string]$Label) {
    ...
    Write-Host "[r3][hash] $Label=$actual"
}

function Invoke-Download([string]$Url, [string]$Path) {
    Write-Host "[r3][download] $Url"
...
New-Item -ItemType Directory -Force -Path $Temp,$PythonRoot,$AsrRoot,$TranslationRoot,$VoiceSource,$VbPackage | Out-Null

Write-Host '[r3] Stage CPython 3.12.10 embeddable runtime'
...
Write-Host "[r3][python] archive_md5=$pythonMd5 archive_sha256=$pythonSha bytes=$((Get-Item $pythonZip).Length)"
...
Write-Host '[r3] Build frozen WorkerRuntime dependency environment'
python -m pip install --disable-pip-version-check uv==0.12.0 huggingface-hub==0.36.2 hf-xet==1.6.0
...
& $privatePython -s -c "import torch, torchaudio, transformers, ctranslate2, faster_whisper, sentencepiece, soundfile, numpy; print('[r3][python] private imports PASS', torch.__version__, transformers.__version__)"
if ($LASTEXITCODE -ne 0) { throw 'Private Python runtime import smoke failed.' }

Write-Host '[r3] Download exact Hugging Face release snapshots'
...
jobs = [
    ...
    ('lj1995/GPT-SoVITS','336b2ec4e8d4ac74740798dd40af44e74659ecaf',os.environ['GPT_ASSET_OUT'],[
        's1v3.ckpt','sv/pretrained_eres2netv2w24s4ep4.ckpt','v2Pro/s2Dv2ProPlus.pth','v2Pro/s2Gv2ProPlus.pth',
        'chinese-hubert-base/**','chinese-roberta-wwm-ext-large/**'
    ]),
]
for repo,rev,out,patterns in jobs:
    print(f'[r3][hf] {repo}@{rev} -> {out}', flush=True)
    snapshot_download(repo_id=repo, revision=rev, local_dir=out, allow_patterns=patterns)
    cache=os.path.join(out,'.cache')
    if os.path.isdir(cache):
        import shutil; shutil.rmtree(cache)
print('[r3][hf] snapshots PASS')
'@ | Set-Content -LiteralPath (Join-Path $Temp 'download_hf.py') -Encoding utf8
python (Join-Path $Temp 'download_hf.py')
if ($LASTEXITCODE -ne 0) { throw 'Hugging Face snapshot staging failed.' }

Write-Host '[r3] Stage pinned GPT-SoVITS source revision'
...
Write-Host '[r3] Stage pinned GPT-SoVITS pretrained assets'
...
Write-Host '[r3] Stage exact NLTK data snapshot'
...
foreach ($item in @(
    ...
    $sha=Get-Sha256 $zip
    $nltkRecords += "$category/$name.zip sha256=$sha"
...
Get-Content -LiteralPath (Join-Path $VoiceSource 'NLTK_DATA_SOURCE.txt') | ForEach-Object { Write-Host "[r3][nltk] $_" }

Write-Host '[r3] Stage pinned FFmpeg LGPL executable and source companions'
...
Write-Host '[r3] Stage exact official VB-CABLE Pack45'
...
Write-Host '[r3] Remove transient download metadata from staged runtime'
...
Write-Host '[r3] Static VoiceLab source-asset preflight through private Python'
$env:PYTHONPATH=$Worker
$env:SOURCE_ROOT=$VoiceSource
& $privatePython -s -c "from pathlib import Path; import os; import voice_lab_gpt_sovits as v; a=v.source_assets(Path(os.environ['SOURCE_ROOT'])); print('[r3][voice] source assets PASS', sorted(a.keys()))"
if ($LASTEXITCODE -ne 0) { throw 'VoiceLab staged source asset validation failed.' }

Write-Host '[r3] Staged payload sizes'
foreach ($pair in @(
    ...
    Write-Host "[r3][size] $($pair[0]) bytes=$bytes"
}

Write-Host '[r3] controlled release input staging -> PASS'
