...
ROOT = Path("EngineData/Backend/LocalWorker/PythonRuntime").resolve()
...
def download(url: str) -> bytes:
    print(f"[r3][license-stage] download {url}", flush=True)
...
def write_material(root: Path, name: str, data: bytes, expected: str) -> None:
    ...
    print(f"[r3][license-stage] {root.parent.name}/{name} sha256={actual}", flush=True)
...
def raw_package(dist: str, source_url: str, material_name: str, expected: str, source_lines: list[str]) -> None:
    root = dist_info(dist)
...
def verify_sdist(url: str, expected: str) -> bytes:
    ...
    print(f"[r3][license-stage] sdist sha256={actual}", flush=True)
    return data


def sdist_package(dist: str, url: str, archive_hash: str, material: dict[str, tuple[str, str]], source_lines: list[str]) -> None:
...
verify_sdist(jieba_sdist, "055ca12f62674fafed09427f176506079bc135638a14e23e25be909131928db2")
raw_package(
...
verify_sdist(jieba_fast_sdist, "e92089d52faa91d51b6a7c1e6e4c4c85064a0e36f6a29257af2254b9e558ddd0")
raw_package(
...
verify_sdist(loguru_sdist, "19480589e77d47b8d85b2c827ad95d49bf31b0dcde16593892eb51dd18706eb6")
raw_package(
...
write_material(
    onnx_root,
    "LICENSE",
    download("https://raw.githubusercontent.com/microsoft/onnxruntime/da9b5e364c465de65c49d91e696cd6485270757f/LICENSE"),
    "2f07c72751aed99790b8a4869cf2311df85a860b22ded05fa22803587a48922c",
)
write_material(
    onnx_root,
    "ThirdPartyNotices.txt",
    download("https://raw.githubusercontent.com/microsoft/onnxruntime/da9b5e364c465de65c49d91e696cd6485270757f/ThirdPartyNotices.txt"),
...
sdist_package(
    ...
    {
        "LICENSE": ("sentencepiece-0.2.2/sentencepiece/LICENSE", "cfc7749b96f63bd31c3c42b5c471bf756814053e847c10f3eb003417bc523d30"),
        "ABSEIL_LICENSE": ("sentencepiece-0.2.2/sentencepiece/third_party/abseil-cpp/LICENSE", "c79a7fea0e3cac04cd43f20e7b648e5a0ff8fa5344e644b0ee09ca1162b62747"),
        "DARTS_CLONE_LICENSE": ("sentencepiece-0.2.2/sentencepiece/third_party/darts_clone/LICENSE", "155f59997298ee336602c49f9c1110f268ac394ca2197eb02647a3555935ad52"),
        "ESAXX_LICENSE": ("sentencepiece-0.2.2/sentencepiece/third_party/esaxx/LICENSE", "7c28553d1d3312d65fe309f76a22ebaf33a3d76c8c1e3b98a88ee4654ecb53db"),
        "PROTOBUF_LITE_LICENSE": ("sentencepiece-0.2.2/sentencepiece/third_party/protobuf-lite/LICENSE", "6e5e117324afd944dcf67f36cf329843bc1a92229a8cd9bb573d7a83130fea7d"),
    },
    [
...
sdist_package(
    ...
    {"LICENSE": ("tokenizers-0.21.4/tokenizers/LICENSE", "c71d239df91726fc519c6eb72d318ec65820627232b2f796219e87dcf35d0ab4")},
    [
...
sdist_package(
    ...
    {"LICENSE": ("wordsegment-1.3.1/LICENSE", "8fe4d37c518608a57c6b0f24e26915144f8daf460eb4e1572146596ec3673294")},
    [
...
print("[r3][license-stage] reviewed exceptional Python license materials -> PASS")
